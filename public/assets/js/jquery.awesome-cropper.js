...
(function() {
	...
	$ = jQuery;
  	$.awesomeCropper = function(inputAttachTo, options) {
	    ...
	    $container.css({
			"width" : settings.width_view,
			"height": settings.height_view
	    });

	    if(settings.image_cover){
	    	$container.css({
				'background' : 'url("'+settings.image_cover+'")',
				'background-size': '100%'
		    });
	    }
	    ...
	    saveCrop = function() {
			var result;
			result = $cropSandbox.get(0).toDataURL("image/jpeg", 0.5); // convert image to jpeg
